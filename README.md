...
A few benchmarks span several distinct domains that share only a paper. Those
get a parent folder containing one sub-folder per domain, each of which is an
ordinary benchmark module in its own right. `DreamCoder_2021` is one such
benchmark, with sub-benchmarks for list processing, text editing, LOGO
graphics, block towers and physical laws:

```julia,jldoctest
julia> using HerbBenchmarks

julia> import HerbBenchmarks.DreamCoder_2021: Tower_2021

julia> length(get_all_problems(Tower_2021))
113
```
