...
function interpret(prog::AbstractRuleNode, grammartags::Dict{Int,Symbol}, state::StringState)
    ...
    @match grammartags[rule_node] begin
        ...
        :drop => state.pointer < length(state.string) ? StringState(state.str[1:state.pointer-1] * state.str[state.pointer+1:end], state.pointer) : StringState(state.str[1:state.pointer-1] * state.str[state.pointer+1:end], state.pointer-1) #drop
